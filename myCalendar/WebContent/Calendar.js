...
function controller(target) {
	...
	this.renderCalendar = function() {
		...
		arrTable.push('</colgroup><thead><tr style="text-align:center">');
		...
		for(var i=0; i<100; i++) {
			...
			sClass += i % 7 == 0 ? 'sun' : '';
			sClass += i % 7 == 6 ? 'sat' : '';
			sClass += oStartDt - today == 0 ? 'today' : '';
			
			//날짜 yyyy-MM-dd형식으로 변환
			var year = oStartDt.getFullYear();
			var month = oStartDt.getMonth();
			if(month<10) month = '0' + month;
			var day = oStartDt.getDate();
			if(day<10) day = '0' + day;			

			var DateFormat = year + "-" + month + "-" + day;

			/*
			 * db에서 시간값에서 날짜 부분만 추출하여 DateFormat값과 비교
			 * 값이 같으면 db에서 같은 날짜로 예약된 이름 개수 추출하여
			 * 예약 n개 식으로 출력
			 */
			
			//각 열마다 날짜를 표시
			//(예: 9월 달력에서 8월 27일은 일요일이므로 <td class="date-cell not-this-month sun">27</td>로 표시될 것이다)
			arrTable.push('<td class="'+sClass+'">' + oStartDt.getDate() + '</br></td>');
	...
	this.initEvent = function() {
		...
		$('#btnToday').click(that.onTodayCalendar);
	}
	...
	/* 오늘 날짜로 이동 */
	this.onTodayCalendar = function(){ //m_oMonth값을 오늘 날짜로 재지정하여 렌더링
		m_oMonth.setFullYear(today.getFullYear());
		m_oMonth.setMonth(today.getMonth());
		m_oMonth.setDate(today.getDate());
		that.renderCalendar();
	}
		
	this.CreateEvent = function(oStartDt){
		var year = oStartDt.getFullYear();
		var month = oStartDt.getMonth();
		var day = oStartDt.getDate();
